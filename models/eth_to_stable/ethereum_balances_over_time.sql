...
with double_entry_book as (
    ...
    and to_address in (select distinct(borrower) from {{ref('compound_usdc_sample_address')}})
    and status = 1
    ...
    and from_address in (select distinct(borrower) from {{ref('compound_usdc_sample_address')}})
    and status = 1
    and (call_type not in ('delegatecall', 'callcode', 'staticcall') or call_type is null)
    union all
    ...
    where miner in (select distinct(borrower) from {{ref('compound_usdc_sample_address')}})
    ...
    where from_address in (select distinct(borrower) from {{ref('compound_usdc_sample_address')}})
),
